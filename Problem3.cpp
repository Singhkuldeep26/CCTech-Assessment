...
public:
    ...
    int findPointToRemoveForLargestArea() {
        double maxArea = largestArea();
        int pointToRemove = -1;

        for (int i = 0; i < points.size(); i++) {
            std::vector<Point> reducedPoints = points;
            reducedPoints.erase(reducedPoints.begin() + i);
            ConvexHull convexHull(reducedPoints);
            double area = convexHull.largestArea();
            if (area > maxArea) {
                maxArea = area;
                pointToRemove = i;
            }
...
int main() {
    ...
    int pointToRemove = convexHull.findPointToRemoveForLargestArea();

    if (pointToRemove != -1) {
        std::cout << "Remove point at index " << pointToRemove << " to get the largest area." << std::endl;
    } else {
        std::cout << "No point needs to be removed to maximize the area." << std::endl;
    }
